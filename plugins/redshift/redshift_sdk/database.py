"""Database Module. Redshift functionalities"""
...
from aws_orbit_sdk.common import ORBIT_ENV, ORBIT_TEAM_SPACE, get_properties, split_s3_path
from aws_orbit_sdk.database import DatabaseCommon
...
class RedshiftUtils(DatabaseCommon):
    ...
    def get_connection_to_redshift(
        self, clusterIdentifier: str, DbName: str, DbUser: str, lambdaName: Optional[str] = None
        ...
        To connect to existing cluster:
            **Cluster networking is configured correctly (has access to the Orbit VPC).
            **Cluster ESG was created, assigned to the cluster.
            **The ESG should have an ingress rule to allow inbound tcp on the redshift port from the Orbit
                instance SG.
        ...
        if lambdaName:
            ...
            try:
                ...
                response = lambda_client.invoke(
                    ...
                    Payload=json.dumps({"DbUser": DbUser, "DbName": DbName, "clusterIdentifier": clusterIdentifier}),
                )
            ...
            response = redshift.get_cluster_credentials(
                DbUser=DbUser, DbName=DbName, AutoCreate=True, ClusterIdentifier=clusterIdentifier
            )
        ...
        return {"db_url": self.db_url, "engine": self.current_engine, "redshift_role": self.redshift_role}
    ...
    def create_external_schema(self, schema_name: str, glue_database: str) -> None:
        ...
        createSchemaSql = f"""create external schema IF NOT EXISTS {schema_name}
    ...
    def create_external_table(
        ...
        ddl = f"""UNLOAD ('{select}') TO '{s3_path}' iam_role '{self.redshift_role}' FORMAT AS {format} {options} """
    ...
    def _get_cred_to_redshift_cluster(self, cluster_name: str) -> Dict[str, str]:
        ...
        return {"user": response_payload["user"], "password": response_payload["password"]}
    ...
    def _start_and_wait_for_redshift(
        self, redshift: botocore.client.Redshift, cluster_name: str, props: Dict[str, str], clusterArgs: Dict[str, str]
    ) -> Tuple[str, str]:
        """
        ...
        if response_payload["modify_status"]:
            logger.info("resuming existing cluster: %s", cluster_id)
        else:
    ...
    def _add_json_schema(
        self, s3: botocore.client.s3, glue: botocore.client.glue, table: str, database_name: str, table_name: str
    ) -> None:
        """
        ...
        response = glue.get_table(DatabaseName=database_name, Name=table_name)
        # This is another way to get the properties. however, for now we will stick with the glue way that can also work in the future for Athena # noqa
        # """
        ...
        #         FROM pg_get_external_tables() ext_tables(esoid integer, schemaname character varying, tablename character varying, "location" character varying, input_format character varying,  # noqa
        #         output_format character varying, serialization_lib character varying, serde_parameters character varying, compressed integer, parameters character varying)  # noqa
        #         ) A
        #         LIMIT 10
        #         ;
        #
    ...
    def getCatalog(
        ...
        sql = """
                    ...
                    FROM SVV_EXTERNAL_COLUMNS cols 
                     natural join SVV_EXTERNAL_TABLES tables natural join SVV_EXTERNAL_SCHEMAS schemas 
                """  # noqa
    ...
    def get_team_clusters(
        ...
        for cluster in clusters:
            ...
            cluster_nodes_info = {"node_type": cluster["NodeType"], "nodes": len(cluster["ClusterNodes"])}
