...
def destroy(manifest: Manifest, filename: str) -> Manifest:
    stack_name: str = f"datamaker-{manifest.name}"
    final_eks_stack_name: str = f"eksctl-{stack_name}-cluster"
    _logger.debug("EKSCTL stack name: %s", final_eks_stack_name)
    ...
    if does_cfn_exist(stack_name=final_eks_stack_name):
        ...
        with start_spinner(msg="Synthetizing the EKSCTL manifest") as spinner:
            output_filename = generate_manifest(manifest=manifest, filename=filename, name=stack_name)
            spinner.succeed()

        with start_spinner(msg="Destroying EKSCTL resources") as spinner:
            output_filename = (
                f"{path_from_filename(filename=filename)}.datamaker.out/{manifest.name}/eksctl/cluster.yaml"
            )
