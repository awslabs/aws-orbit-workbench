...
class StateMachineBuilder:
    ...
    def build_eks_run_container_state_machine(
        ...
        job: Dict[str, Any] = {
            ...
            "kind": "Job",
            "metadata": {
                "labels": {"app": f"datamaker-{team_manifest.name}-{node_type}-runner"},
                "generateName": f"datamaker-{team_manifest.name}-{node_type}-runner-",
    ...
    def build_eks_k8s_api_state_machine(
        ...
        return sfn.StateMachine(
            ...
            state_machine_name=f"datamaker-{manifest.name}-{team_manifest.name}-eks-k8s-api",
