...
import socket
from ..common import *
from typing import Optional, Mapping, Tuple, Union, Dict, Any, List
...
def _start_and_wait_for_emr(
    ...
    response = sfn.start_execution(
        stateMachineArn=emr_function['StateMachine'],
        input=json.dumps(emr_input)
    )
    logger.debug(f"Started EMR step function {response}")

    while True:
        time.sleep(20)
        des_res = sfn.describe_execution(
...
def get_cluster_info(cluster_id: str) -> Dict[str, Dict[str, str]]:
    ...
    info['CORE'] = emr.list_instances(
        ClusterId=cluster_id,
        InstanceGroupTypes=[
            'CORE'
        ]
    )
    info['TASK'] = emr.list_instances(
        ClusterId=cluster_id,
        InstanceGroupTypes=[
            'TASK'
        ]
...
def spark_submit(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    s3WorkspaceDir = "s3://{}/{}/workspaces/{}".format(props['AWS_DATAMAKER_S3_BUCKET'], props['DATAMAKER_TEAM_SPACE'],
...
def get_team_clusters(cluster_id: Optional[str] = None) -> Dict[str, Dict[str, str]]:
    ...
    for cluster in clusters:
        ...
        if tags[DATAMAKER_ENV] != props['AWS_DATAMAKER_ENV'] or tags[DATAMAKER_TEAM_SPACE] != props[
            'DATAMAKER_TEAM_SPACE']:
            continue
