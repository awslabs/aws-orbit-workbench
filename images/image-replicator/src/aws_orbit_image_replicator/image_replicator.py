...
def _generate_build_spec(repo_host: str, src: str, dest: str) -> Dict[str, Any]:
    ...
    build_spec = {
        ...
        "phases": {
            ...
            "pre_build": {"commands": [
                ...
                f"aws ecr create-repository --repository-name {repo} || echo 'Already exists'"
            ]},
            "build": {"commands": [
                f"docker pull {src}",
...
def _replicate_image(config: Dict[str, Any], src: str, dest: str) -> str:
    ...
    build_id = client.start_build(
        ...
        buildspecOverride=yaml.safe_dump(_generate_build_spec(config["repo_host"], src, dest)),
...
def replicate(
    ...
    while True:
        try:
            ...
            replication_task = replications_queue.get(block=True, timeout=None)
            ...
            with lock:
                if result == "SUCCEEDED":
                    ...
                    logger.error("Image Replication Attempt %s Failed: %s -> %s", attempt, replication_task["src"], replication_task["dest"])
